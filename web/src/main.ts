...
const data=[
  {slug:'wild-dog-milk',hash:'eternity',title:'永恒',en:'ETERNITY',subtitle:'宇宙有终点，它没有。',description:'星系诞生、膨胀，最终归于寂灭。<br>下一次宇宙醒来，它仍在原处。',artifact:'野生狗奶',spec:'保质期：永久',accent:'#a6d3ff',tint:'#548ccd',symbol:'∞',coordinate:'存在，无需期限',action:'推进宇宙生灭',hint:'拖动旋转 · 滚轮缩放 · 开启轮回'},
  {slug:'chrysanthemum-drink',hash:'space',title:'空间',en:'SPACE',subtitle:'方寸之间，容纳十倍世界。',description:'2500 毫升，挤进一盒寻常的大小。<br>把折叠的十份空间，从包装里展开。',artifact:'菊花饮料',spec:'净含量：2500 毫升',accent:'#b4edcb',tint:'#34b983',symbol:'↗',coordinate:'距离，不过一种错觉',action:'展开十倍空间',hint:'拖动旋转 · 滚轮缩放 · 展开空间'},
  {slug:'chunqiu-sausage',hash:'time',title:'时间',en:'TIME',subtitle:'先有终点，才有开始。',description:'一分钟走过三十年，十年倒欠保质期。<br>它在诞生之前，已经过期了一个年代。',artifact:'春秋肠',spec:'2018 生产 · 2008 保质',accent:'#edbd88',tint:'#c96a40',symbol:'↶',coordinate:'先有过去，还是未来',action:'',hint:'拖动旋转 · 滚轮缩放 · 拨动因果'},
];
type ParticleMesh=ReturnType<typeof particles>;
type World={scene:THREE.Scene;anchor:THREE.Group;model:THREE.Group;asset:THREE.Object3D;surface:ReturnType<typeof sampleSurface>;systems:ParticleMesh[];haze:ReturnType<typeof atmosphere>;rings?:THREE.Group;ribbon?:ReturnType<typeof flowRibbon>;dial?:THREE.LineSegments;portals:THREE.Group[];echoes:THREE.Group[];materials:{material:THREE.MeshStandardMaterial;opacity:number}[];clock:number;universe?:THREE.Group;chronology?:ReturnType<typeof chronology>};
...
let viewDistance=10.4,eternalCycle=!reduced,cycleClock=0,cycleCount=0,spaceTarget=0,spaceSpread=0,immersive=false;
const renderedViews=views.map(v=>({...v}));
let immersion=0;
...
function resize(){
  ...
  worlds.forEach(w=>{if(!w)return;w.systems.forEach(p=>{p.material.uniforms.uDpr.value=currentDpr;const count=p.geometry.attributes.position.count;p.geometry.setDrawRange(0,lowQuality?Math.floor(count*.48):count);});w.surface.material.uniforms.uDpr.value=currentDpr;w.surface.geometry.setDrawRange(0,lowQuality?12000:36000);});
}
...
async function loadWorld(index:number):Promise<World>{
  ...
  const promise=(async()=>{
    ...
    if(index===0){
      const universe=new THREE.Group();world.universe=universe;anchor.add(universe);universe.add(infinityRiver());
      const ringGroup=new THREE.Group();ringGroup.rotation.set(.43,0,.34);
      const dust=particles(32000,0,'#76a8e8','#f1f6ff');ringGroup.add(dust,orbitLines('#9bceff',3.25,10));world.ribbon=flowRibbon(0,'#b2d5ff');ringGroup.add(world.ribbon);universe.add(ringGroup);systems.push(dust);world.rings=ringGroup;
      const distant=orbitLines('#375a88',4.8,2);distant.rotation.set(-.36,.25,-.6);distant.position.z=-3;universe.add(distant);
    }else if(index===1){
      ...
      for(let i=0;i<10;i++){
        ...
        if(i!==0){const echo=inspectionCopy(normalizer);echo.userData.baseScale=normalizer.scale.x;echo.rotation.y=i===0?Math.PI*.65:-Math.PI*.6;anchor.add(echo);echoes.push(echo);}
      }
      ...
      world.chronology=chronology();anchor.add(world.chronology.root);
...
function travel(){
  ...
  $('#action-label').textContent=spaceTarget?'折回同一空间':'展开十倍空间';
}
function release(){if(!transitioning)targetDissolve=0;}
function syncCycle(){$('#cycle').setAttribute('aria-pressed',String(eternalCycle));$('#cycle').textContent=eternalCycle?'暂停宇宙演化 ∞':'继续宇宙演化 ∞';}
$('#cycle').onclick=()=>{eternalCycle=!eternalCycle;syncCycle();};syncCycle();
$('#space-spread').addEventListener('input',()=>{spaceTarget=Number($<HTMLInputElement>('#space-spread').value);$('#action-label').textContent=spaceTarget>.5?'折回同一空间':'展开十倍空间';});
$('#action').addEventListener('click',()=>{if(current===1)travel();else if(current===0){eternalCycle=true;cosmicSpeed=5;syncCycle();}});
let cosmicSpeed=1;
...
function resetView(){
  const actual=renderedViews[current];
  // Choose the nearest full turn so reset never spins through accumulated revolutions.
  actual.yaw=THREE.MathUtils.euclideanModulo(actual.yaw+Math.PI,Math.PI*2)-Math.PI;
  views[current]={yaw:0,pitch:0,distance:10.4};
}
...
function setImmersive(value:boolean){
  ...
  document.querySelectorAll<HTMLElement>('.site-header,main,.site-footer').forEach(el=>{el.inert=value;});
  if(document.activeElement instanceof HTMLElement)document.activeElement.blur();
  $('#immersive').setAttribute('aria-pressed',String(value));
  clearTimeout(hintTimeout);$('#immersive-hint').classList.toggle('show',value);
  if(value)hintTimeout=setTimeout(()=>$('#immersive-hint').classList.remove('show'),3000);
}
let lastDouble=0;
function doubleAction(){const now=performance.now();if(now-lastDouble<400)return;lastDouble=now;if(immersive)setImmersive(false);else resetView();}
stage.addEventListener('dblclick',doubleAction);
let tapStart={x:0,y:0,t:0},lastTap=0,tapMoved=false;
stage.addEventListener('pointerdown',e=>{if(e.pointerType!=='touch')return;tapStart={x:e.clientX,y:e.clientY,t:performance.now()};tapMoved=touches.size>1;});
stage.addEventListener('pointermove',e=>{if(Math.hypot(e.clientX-tapStart.x,e.clientY-tapStart.y)>12)tapMoved=true;});
stage.addEventListener('pointerup',e=>{if(e.pointerType!=='touch'||tapMoved)return;const now=performance.now();if(now-tapStart.t>260)return;if(now-lastTap<300){doubleAction();lastTap=0;}else lastTap=now;});
$('#immersive').onclick=()=>setImmersive(!immersive);
...
addEventListener('keydown',event=>{if(event.target instanceof HTMLInputElement||event.target instanceof HTMLButtonElement||event.target instanceof HTMLAnchorElement)return;if(event.key==='ArrowRight')void switchScene((current+1)%3);if(event.key==='ArrowLeft')void switchScene((current+2)%3);if(event.key==='Escape'){setImmersive(false);release();}});
...
function frame(now:number){
  ...
  cosmicSpeed=THREE.MathUtils.damp(cosmicSpeed,1,.55,dt);
  if(current===0&&eternalCycle)cycleClock+=dt*cosmicSpeed;
  const universeAge=(cycleClock%36)/36;
  cycleCount=Math.floor(cycleClock/36);
  if(current===0)$('#epoch').textContent=`宇宙 ${cycleCount+1} · ${universeAge<.2?'诞生':universeAge<.65?'膨胀':universeAge<.88?'热寂':'下一次创生'} / 牛奶：永久`;
  ...
  if(current===2){
    ...
    const year=Math.floor(2000+age*30);world.chronology?.update(age);world.chronology?.root.scale.setScalar(isMobile()?.6:1);$('#coordinate-value').textContent='−10 年';
  ...
  monuments.get(current)?.update(t,current===0?universeAge:current===2?age:dissolve);
  if(world.universe){const life=Math.pow(Math.sin(Math.PI*universeAge),2);world.universe.scale.setScalar(.15+3.4*life);world.universe.visible=life>.002;}
  spaceSpread=THREE.MathUtils.damp(spaceSpread,spaceTarget,2.2,dt);
  if(current===1)$('#space-value').textContent=`容积 × ${(1+spaceSpread*9).toFixed(1)} · ${Math.round(250+spaceSpread*2250)} mL`;
  ...
  const view=renderedViews[current];
  view.yaw=THREE.MathUtils.damp(view.yaw,views[current].yaw,touches.size?18:5,dt);
  view.pitch=THREE.MathUtils.damp(view.pitch,views[current].pitch,touches.size?18:5,dt);
  immersion=THREE.MathUtils.damp(immersion,immersive?1:0,5,dt);
  world.anchor.position.set((isMobile()?0:1.85)*(1-immersion),(isMobile()?1.55:.05)*(1-immersion),0);
  world.anchor.scale.setScalar(THREE.MathUtils.lerp(isMobile()?.64:1,isMobile()?.78:1,immersion));
  world.model.position.set(0,current===0?0:Math.sin(t*.45)*.07,0);
  world.model.rotation.set(current===2?.29:.04,current===2?-.1+Math.sin(t*.16)*.04:current===0?.32:.32+Math.sin(t*.22)*.10,current===2?-.11+Math.sin(t*.2)*.025:current===0?.035:.035+Math.sin(t*.3)*.017);
  world.model.rotation.x+=view.pitch;world.model.rotation.y+=view.yaw;
  if(world.rings){world.rings.rotation.z=.34+Math.sin(t*.12)*.035;world.rings.rotation.y=Math.sin(t*.09)*.06;}
  world.systems.forEach(p=>{p.material.uniforms.uTime.value=t;p.material.uniforms.uForce.value=dissolve*.35;p.material.uniforms.uLife.value=current===0?Math.pow(Math.sin(Math.PI*universeAge),2):1;});world.haze.material.uniforms.uTime.value=t;if(current===0)world.haze.material.uniforms.uColor.value.set(data[0].tint).multiplyScalar(Math.pow(Math.sin(Math.PI*universeAge),2));
  ...
  world.echoes.forEach((echo,i)=>{
    if(current===2){const past=t-(i-3)*5;echo.rotation.set(.29+view.pitch,-.1+view.yaw+Math.sin(past*.16)*.04,-.11+Math.sin(past*.2)*.025);echo.position.y=-1.2-Math.abs(i-3)*.15+Math.sin(past*.2)*.12;}
    else{
      const unit=i+1,angle=(unit-1)/9*Math.PI*2;
      echo.visible=spaceSpread>.025;
      echo.position.set(Math.cos(angle)*7.4*spaceSpread,Math.sin(angle)*4.4*spaceSpread,-3.5);
      echo.rotation.set(view.pitch,view.yaw+angle,.03);
      echo.scale.setScalar(echo.userData.baseScale*(.3+.45*spaceSpread));
    }
  });
  world.portals.forEach((portal,i)=>{
    const angle=(i-1)/9*Math.PI*2;
    portal.position.set(i===0?0:Math.cos(angle)*7.4*spaceSpread,i===0?0:Math.sin(angle)*4.4*spaceSpread,i===0?-2:-4.2);
    portal.scale.setScalar(i===0?1:.75);portal.visible=i===0||spaceSpread>.025;
  ...
  const extraDistance=current===1?spaceSpread*11.5:0;
  ...
  Object.assign(diagnostics,{scene:current,ready,transitioning,dissolve,timePosition:(timePosition%60+60)%60,continuousTime:timePosition,timeRate,portalCount,portalTravel,particles:world.systems.reduce((n,p)=>n+p.geometry.drawRange.count,0),fps:Math.round(fps),loaded:worlds.filter(Boolean).length,drawCalls:renderer.info.render.calls,music:music.state,view:{...views[current],actualYaw:view.yaw,actualPitch:view.pitch,actualDistance:viewDistance},immersive,immersion,universeAge,artifactPosition:world.model.position.toArray(),artifactRotation:world.model.rotation.toArray(),artifactOpacity:visible,spatialUnits:world.portals.length,volumeFactor:1+9*spaceSpread,spaceSpread,eternalCycle,cycleCount,age});
}
